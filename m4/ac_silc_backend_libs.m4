...
dnl use AC_SILC_BACKEND_LIB_OPTIONS. It will provide the configure options
dnl --with-backend-<libname>-includes and --with-backend-<libname>-lib.

dnl The macro AC_SILC_BACKEND_LIB_REQUIRE_ARGUMENTS will determine if the user
dnl needs to specify the paths to the lib and the includes (on cross compile
dnl systems)

dnl The macro AC_SILC_BACKEND_LIB_HEADER_USABLE will check if the requested header
dnl files are usable.

dnl Following variables are of interest so far ($1 is the name of the lib):
dnl $ac_silc_with_backend_$1_includes
dnl $ac_silc_with_backend_$1_lib
dnl $ac_silc_check_for_backend_$1
dnl $ac_silc_has_backend_$1_includes
dnl m4_join([_], [LIB], m4_toupper([$1]), [CPPFLAGS])
dnl m4_join([_], [HAVE_LIB], m4_toupper([$1])

dnl There will be a macro AC_SILC_BACKEND_LIB_USABLE that will try to link a small
dnl test program. Needs to be implemented.

dnl A user can then build an own macro using:
dnl AC_SILC_BACKEND_LIB_OPTIONS([cubew4])
dnl AC_SILC_BACKEND_LIB_REQUIRE_ARGUMENTS([cubew4])
dnl AC_SILC_BACKEND_LIB_HEADER_USABLE([cubew4], [cubew.h])
dnl AC_SILC_BACKEND_LIB_USABL([cubew4], ...)


AC_DEFUN([AC_SILC_BACKEND_LIB_OPTIONS], [
...
AC_ARG_WITH([backend-$1-includes],
            ...
            [ac_silc_with_backend_$1_includes=$withval],
            [ac_silc_with_backend_$1_includes=""])

AC_ARG_WITH([backend-$1-lib],
            [AS_HELP_STRING([--with-backend-$1-lib=path], 
                            [Path to lib$1 (a backend version on cross-compile systems)])],
            [ac_silc_with_backend_$1_lib=$withval],
            [ac_silc_with_backend_$1_lib=""])

])

#------------------------------------------------------------------------------

AC_DEFUN([AC_SILC_BACKEND_LIB_HEADER_USABLE], [
# arg1: lib name as in AC_SILC_BACKEND_LIB_OPTIONS, e.g. cubew4
# arg2-n: name of includes, e.g. cubew.h

if test "x${ac_silc_check_for_backend_$1}" = "yes"; then

    AC_LANG_PUSH([C])
    cppflags_save="$CPPFLAGS"

    user_provided_cppflags=""
    if test "x${ac_silc_with_backend_$1_includes}" != "x"; then
        user_provided_cppflags="-I${ac_silc_with_backend_$1_includes}"
    ...
    ac_silc_has_backend_$1_includes="no"
    if  test "x$has_header" = "xyes"; then
        ac_silc_has_backend_$1_includes="yes"
    fi
    ...
    AC_MSG_RESULT([$ac_silc_has_backend_$1_includes])
...
AC_DEFUN([AC_SILC_BACKEND_LIB_REQUIRE_ARGUMENTS], [
# arg1: lib name as in AC_SILC_BACKEND_LIB_OPTIONS, e.g. cubew4
ac_silc_check_for_backend_$1="yes"
if test "x${ac_silc_cross_compiling}" = "xyes"; then
    if test "x${ac_silc_with_backend_$1_includes}" = "x" -o "x${ac_silc_with_backend_$1_lib}" = "x"; then
        ...
        ac_silc_check_for_backend_$1="no"
    fi
fi
# needs to be set in AC_SILC_BACKEND_LIB_USABLE. It is here just to please autoreconf
AM_CONDITIONAL(m4_join([_], [HAVE_LIB], m4_toupper([$1])), [test 1 -eq 1])
])

#------------------------------------------------------------------------------

dnl AC_DEFUN([AC_SILC_BACKEND_LIB_USABLE], [
dnl # arg1: lib name as in AC_SILC_BACKEND_LIB_OPTIONS, e.g. cubew4
