...
RESULT_DIR=scorep-hybrid-papi-metric-test-dir
rm -rf $RESULT_DIR
...
if diff $TEST_DATA_DIR/jacobi_c_hybrid_papi_metric_definitions.out trace.txt > /dev/null
  then
    ...
    diff -u $TEST_DATA_DIR/jacobi_c_hybrid_papi_metric_definitions.out trace.txt >&2
    rm trace.txt
    exit 1
fi
...
if [ x`grep -c METRIC trace.txt` = x`grep -c METRIC $TEST_DATA_DIR/jacobi_c_hybrid_papi_metric_events.out` ]; then
    rm trace.txt
    exit 0
  else
    echo "-------- ERROR: in metric event output --------" >&2
    diff -u $TEST_DATA_DIR/jacobi_c_hybrid_papi_metric_events.out trace.txt >&2
    rm trace.txt
    exit 1
fi
