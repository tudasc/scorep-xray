...
RESULT_DIR=$PWD/scorep-mpi-papi-metric-test-dir
rm -rf $RESULT_DIR

# Run test
SCOREP_EXPERIMENT_DIRECTORY=$RESULT_DIR SCOREP_ENABLE_PROFILING=false SCOREP_ENABLE_TRACING=true SCOREP_METRIC_PAPI_SEP=, SCOREP_METRIC_PAPI=PAPI_TOT_INS $MPIRUN -np 4 ./jacobi_mpi_c_metric_test
if [ $? -ne 0 ]; then
...
if diff $TEST_DATA_DIR/jacobi_c_mpi_papi_metric_definitions.out trace.txt > /dev/null
  then
    ...
    diff -u $TEST_DATA_DIR/jacobi_c_mpi_papi_metric_definitions.out trace.txt >&2
    rm trace.txt
    exit 1
fi
...
if [ x`grep -c METRIC trace.txt` = x`grep -c METRIC $TEST_DATA_DIR/jacobi_c_mpi_papi_metric_events.out` ]; then
    ...
    diff -u $TEST_DATA_DIR/jacobi_c_mpi_papi_metric_events.out trace.txt >&2
    rm trace.txt
    exit 1
fi
