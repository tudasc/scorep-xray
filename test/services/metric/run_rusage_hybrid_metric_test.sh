...
# Run test
SCOREP_EXPERIMENT_DIRECTORY=$RESULT_DIR OMP_NUM_THREADS=2 SCOREP_ENABLE_PROFILING=false SCOREP_ENABLE_TRACING=true SCOREP_METRIC_RUSAGE_SEP=, SCOREP_METRIC_RUSAGE=ru_utime,ru_stime $MPIRUN -np 2 ./jacobi_mpi_omp_c_metric_test
if [ $? -ne 0 ]; then
...
if diff $TEST_DATA_DIR/jacobi_c_hybrid_rusage_metric_definitions.out trace.txt > /dev/null
  then
    true
  else
    echo "-------- ERROR: in metric definition output --------" >&2
    diff -u $TEST_DATA_DIR/jacobi_c_hybrid_rusage_metric_definitions.out trace.txt >&2
    rm trace.txt
    exit 1
fi
...
if [ x`grep -c METRIC trace.txt` = x`grep -c METRIC $TEST_DATA_DIR/jacobi_c_hybrid_rusage_metric_events.out` ]; then
    rm trace.txt
    exit 0
  else
    echo "-------- ERROR: in metric event output --------" >&2
    diff -u $TEST_DATA_DIR/jacobi_c_hybrid_rusage_metric_events.out trace.txt >&2
    rm trace.txt
    exit 1
fi
