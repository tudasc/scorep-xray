...
## file       run_papi_openmp_metric_test.sh
...
RESULT_DIR=$PWD/scorep-openmp-papi-metric-test-dir
rm -rf $RESULT_DIR

# Run test
SCOREP_EXPERIMENT_DIRECTORY=$RESULT_DIR OMP_NUM_THREADS=4 SCOREP_ENABLE_PROFILING=false SCOREP_ENABLE_TRACING=true SCOREP_METRIC_PAPI_SEP=, SCOREP_METRIC_PAPI=PAPI_TOT_INS ./jacobi_omp_c_metric_test
if [ $? -ne 0 ]; then
...
if diff $TEST_DATA_DIR/jacobi_c_openmp_papi_metric_definitions.out trace.txt > /dev/null
  then
    ...
    diff -u $TEST_DATA_DIR/jacobi_c_openmp_papi_metric_definitions.out trace.txt >&2
    rm trace.txt
    exit 1
fi
...
if [ x`grep -c METRIC trace.txt` = x`grep -c METRIC $TEST_DATA_DIR/jacobi_c_openmp_papi_metric_events.out` ]; then
    rm trace.txt
    exit 0
  else
    echo "-------- ERROR: in metric event output --------" >&2
    diff -u $TEST_DATA_DIR/jacobi_c_openmp_papi_metric_events.out trace.txt >&2
    rm trace.txt
    exit 1
fi
