...
subroutine Init (myData)
    ...
    myData%outer_threads=omp_get_max_threads()
    !$omp parallel
    !$omp master
            myData%inner_threads=omp_get_max_threads()
    !$omp end master
    !$omp end parallel
    write (6,*) 'Jacobi', myData%outer_threads, 'OpenMP-', _OPENMP, ' thread(s)', &
    'on the outer level and ', myData%inner_threads, ' on the inner one'
#elif defined(_OPENMP)
    call omp_set_nested(.true.)
    myData%outer_threads=int(sqrt(real(omp_get_max_threads())))
    myData%inner_threads=omp_get_max_threads()/myData%outer_threads
    write (6,*) 'Jacobi', myData%outer_threads, 'OpenMP-', _OPENMP, ' thread(s)', &
    'on the outer level and ', myData%inner_threads, ' on the inner one'
#else
