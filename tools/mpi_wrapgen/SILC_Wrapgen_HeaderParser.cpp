...
#define SILC_WRAPGEN_MAXLINELENGTH 4096
...
{
    char   cline[ SILC_WRAPGEN_MAXLINELENGTH ];
    string line;
    size_t pos;

    // Read until next ';'
    hfile->getline( cline, SILC_WRAPGEN_MAXLINELENGTH, ';' );
    line = cline;
      ...
      char* argv[] )
{
    ...
    do
    {
        line = SILC::Wrapgen::readLineFromHeader( &file );
        // File end?
        if ( line.empty() )
        {
            break;
        ...
        {
            group =  SILC::Wrapgen::trim( line.substr( 20 ) );
        }
        // Assume function prototype
        else
        {
            SILC::Wrapgen::Func* func = SILC::Wrapgen::evalHeaderLine( line );
