...
)
{
    ...
    if ( thread_index_pointer_array[ 0 ]->shared_index )
    {
        ...
        {
            SCOREP_Hashtab_Free( thread_index_pointer_array[ 0 ]->shared_index->merged_regions_def_table );
        }
    ...
    for ( i = 0; i < thread_count; i++ )
    {
        if ( thread_index_pointer_array[ i ] )
        {
            ...
            {
                SCOREP_Hashtab_Free( thread_index_pointer_array[ i ]->static_measurements_table );
            }
