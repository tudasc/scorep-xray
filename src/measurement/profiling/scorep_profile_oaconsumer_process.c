...
)
{
    ...
    /** Traverese all thread call-trees and copy flat profile measurements into transmittion buffer */
...
)
{
    ...
    {
        int index = get_metric_definition_index( handle );
...
)
{
    ...
    else
    {
        ...
        shared_index->num_counter_definitions    = 0;
    }
}
...
)
{
    ...
    /** Allocate array of thread private index data sctructures, one entry per thread root in profile*/
    ...
    shared_index->rank                     = ( uint64_t )SCOREP_Mpi_GetRank();
    ...
    /** Go through  all thread roots and intialize thread private index data structures */
    scorep_profile_node* thread_root = scorep_profile.first_root_node;
    while ( thread_root != NULL )
    {
        if ( thread_root->node_type == scorep_profile_node_thread_root )
...
//    }
    if ( node->node_type == scorep_profile_node_regular_region )
    {
        ...
        if ( shared_index->dense_metrics_sampling_set )
        {
            ...
            {
                int metric_index = get_metric_definition_index( shared_index->dense_metrics_sampling_set->metric_handles[ i ] );
        ...
        while ( sparse_int != NULL )
        {
            ...
            int metric_index = get_metric_definition_index( sparse_int->metric );
