                          ...
                          MPI_Comm     comm )
{
    ...
    if ( SILC_MPI_IS_EVENT_GEN_ON_FOR( SILC_MPI_ENABLED_COLL ) )
    {
        ...
        SILC_MpiCollective( silc_mpi_regid[ SILC__MPI_REDUCE_SCATTER_BLOCK ],
                            SILC_MPI_COMM_ID( comm ),
                            root_loc,
                            N * recvcount * sz,
                            N * recvcount * sz );
        SILC_ExitRegion( silc_mpi_regid[ SILC__MPI_REDUCE_SCATTER_BLOCK ] );
        SILC_MPI_EVENT_GEN_ON();
    }
    else
    {
                  ...
                  MPI_Op       op )
{
    ...
    {
        SILC_MPI_EVENT_GEN_OFF();
        SILC_EnterRegion( silc_mpi_regid[ SILC__MPI_REDUCE_LOCAL ] );

        return_val = PMPI_Reduce_local( inbuf, inoutbuf, count, datatype, op );

        SILC_ExitRegion( silc_mpi_regid[ SILC__MPI_REDUCE_LOCAL ] );
        SILC_MPI_EVENT_GEN_ON();
    }
    else
    {
