...
/**
 * Declaration of PMPI-symbol for MPI_File_c2f
 */
MPI_Fint
 ...
 * @ingroup io_misc
 * Triggers an enter and exit event.
 * It wraps the MPI_File_c2f call with enter and exit events.
 */
MPI_Fint
MPI_File_c2f( MPI_File file )
{
    SCOREP_IN_MEASUREMENT_INCREMENT();
    const int event_gen_active           = SCOREP_MPI_IS_EVENT_GEN_ON;
    const int event_gen_active_for_group = SCOREP_MPI_IS_EVENT_GEN_ON_FOR( SCOREP_MPI_ENABLED_IO_MISC );
    MPI_Fint  return_val;
    ...
    if ( event_gen_active )
    {
        ...
        if ( event_gen_active_for_group )
        {
            SCOREP_EnterWrappedRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_C2F ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_EnterWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_C2F ] );
        }
    }
    ...
    if ( event_gen_active )
    {
        if ( event_gen_active_for_group )
        {
            SCOREP_ExitRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_C2F ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_ExitWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_C2F ] );
        }
        SCOREP_MPI_EVENT_GEN_ON();
    }
...
/**
 * Declaration of PMPI-symbol for MPI_File_f2c
 */
MPI_File
 ...
 * @ingroup io_misc
 * Triggers an enter and exit event.
 * It wraps the MPI_File_f2c call with enter and exit events.
 */
MPI_File
MPI_File_f2c( MPI_Fint file )
{
    ...
    if ( event_gen_active )
    {
        ...
        if ( event_gen_active_for_group )
        {
            SCOREP_EnterWrappedRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_F2C ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_EnterWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_F2C ] );
        }
    }
    ...
    if ( event_gen_active )
    {
        if ( event_gen_active_for_group )
        {
            SCOREP_ExitRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_F2C ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_ExitWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_FILE_F2C ] );
        }
        SCOREP_MPI_EVENT_GEN_ON();
    }
