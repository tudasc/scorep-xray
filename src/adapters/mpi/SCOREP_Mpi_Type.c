...
/**
 * Declaration of PMPI-symbol for MPI_Sizeof
 */
int
PMPI_Sizeof( MPI_Datatype x,
             int          size );
...
MPI_Sizeof( MPI_Datatype x, int size )
{
    ...
    if ( event_gen_active )
    {
        ...
        {
            SCOREP_EnterWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_SIZEOF ] );
        }
    }
    ...
    if ( event_gen_active )
    {
        ...
        {
            SCOREP_ExitWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_SIZEOF ] );
        }
        SCOREP_MPI_EVENT_GEN_ON();
    }
...
/**
 * Declaration of PMPI-symbol for MPI_Type_c2f
 */
MPI_Fint
 ...
 * @ingroup type_misc
 * Triggers an enter and exit event.
 * It wraps the MPI_Type_c2f call with enter and exit events.
 */
MPI_Fint
MPI_Type_c2f( MPI_Datatype datatype )
{
    SCOREP_IN_MEASUREMENT_INCREMENT();
    const int event_gen_active           = SCOREP_MPI_IS_EVENT_GEN_ON;
    const int event_gen_active_for_group = SCOREP_MPI_IS_EVENT_GEN_ON_FOR( SCOREP_MPI_ENABLED_TYPE_MISC );
    MPI_Fint  return_val;
    ...
    if ( event_gen_active )
    {
        ...
        if ( event_gen_active_for_group )
        {
            SCOREP_EnterWrappedRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_C2F ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_EnterWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_C2F ] );
        }
    }
    ...
    if ( event_gen_active )
    {
        if ( event_gen_active_for_group )
        {
            SCOREP_ExitRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_C2F ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_ExitWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_C2F ] );
        }
        SCOREP_MPI_EVENT_GEN_ON();
    }
...
/**
 * Declaration of PMPI-symbol for MPI_Type_f2c
 */
MPI_Datatype
 ...
 * @ingroup type_misc
 * Triggers an enter and exit event.
 * It wraps the MPI_Type_f2c call with enter and exit events.
 */
MPI_Datatype
MPI_Type_f2c( MPI_Fint datatype )
{
    ...
    if ( event_gen_active )
    {
        ...
        if ( event_gen_active_for_group )
        {
            SCOREP_EnterWrappedRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_F2C ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_EnterWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_F2C ] );
        }
    }
    ...
    if ( event_gen_active )
    {
        if ( event_gen_active_for_group )
        {
            SCOREP_ExitRegion( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_F2C ] );
        }
        else if ( SCOREP_IsUnwindingEnabled() )
        {
            SCOREP_ExitWrapper( scorep_mpi_regions[ SCOREP_MPI_REGION__MPI_TYPE_F2C ] );
        }
        SCOREP_MPI_EVENT_GEN_ON();
    }
