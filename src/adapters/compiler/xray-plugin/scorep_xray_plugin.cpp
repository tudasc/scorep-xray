...
// Try vector of pointers
// Idea: Lookup in array is a bit faster than map
// Idea2: Sorting expensive, but may just insert at right position, but this requires default construction of elements
// to allocate correct size -> pointers lot cheaper and dereferencing is fast
static scorep_compiler_region_description** idToRegion;
...
void buildFunctionMap(std::string &execFileName) XRAY_INSTRUMENT_NEVER {
    ...
    size = funcAddressMap.size();
    idToRegion = new scorep_compiler_region_description*[size];
    ...
    for (auto mapping: funcAddressMap) {
        ...
        if (auto err = maybeFuncInfo.takeError()) {
            ...
            idToRegion[funcId-1] = regionInfo;
